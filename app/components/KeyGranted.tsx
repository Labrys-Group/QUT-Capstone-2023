...
const KeyGranted = ({ status, text, ...props }: ItemTitleProps) => {
  return (
    <Box sx={boxStyle}>
      ...
      <Box>
        <Image name='KeyImage' height='60%' src='/lock.png' alt='Key Image'  
        //OLD TESTING on click change the image to another one
        //onClick={() => {GrantAccess()}}
        />
        

        <Text id='AccessLabel' sx={textStyle} color={'#FFCCCB'} justifyContent={'Center'} fontSize={'14px'}>
          Access denied!
        </Text>
        <PrimaryButton id='AccessButton' text='Purchase for Ξ0.01' 
